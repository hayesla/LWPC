...
for i in loopy:
	...
	f.write('ionosphere homogeneous exponential 0.3 '+ str(i) + '\n')
	...
	print 'h_prime equals' +str(i)

	pha, amp, dist = read_lwpc_log('h_and_beta.log')
	wr = open('h_and_beta.dat', 'a')
	wr.write(str(i) + ' '  + str(pha[-1]) + ' ' + str(amp[-1]) + ' ' + str(dist[-1]) + '\n')
	wr.close()
